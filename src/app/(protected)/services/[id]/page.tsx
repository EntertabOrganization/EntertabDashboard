...
import type { ServiceRequestItem } from "@/lib/api/entertab";
import { deleteServiceRequest, fetchServiceRequest, updateServiceRequest } from "@/lib/api/entertab";
import { getErrorMessage } from "@/lib/getErrorMessage";

export default function ServiceByIdPage() {
  ...
  const [data, setData] = useState<ServiceRequestItem | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const row = await fetchServiceRequest(token, id);
      setData(row);
  ...
  async function handleSave(e: React.FormEvent<HTMLFormElement>) {
    ...
    setSaving(true);
    try {
      const res = await updateServiceRequest(token, id, { name: editName, message: editMessage });
      setData(res.data);
  ...
  return (
    <Card title="Service request">
      <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap", marginBottom: 14 }}>
        <Link href="/services" style={{ ...btn, textDecoration: "none", color: "var(--text)" }}>
          Back
        </Link>
      ...
      {data ? (
        <>
          ...
          <form onSubmit={handleSave} style={{ display: "grid", gap: 12, padding: 14, borderRadius: 14, border: "1px solid var(--border)", background: "rgba(255,255,255,0.75)" }}>
            ...
            <div>
              <div style={{ fontSize: 12, color: "var(--muted)" }}>Update message</div>
              <textarea value={editMessage} onChange={(e) => setEditMessage(e.target.value)} required rows={6} style={{ width: "100%", marginTop: 6, padding: 12, borderRadius: 12, border: "1px solid var(--border)", resize: "vertical" }} />
            </div>
            <button
              type="submit"
